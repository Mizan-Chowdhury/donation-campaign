  ...
  return (
    <div>
      ...
      error ? (<p>{error}</p>
       ...
       seeAll? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-5 px-5 pt-10 lg:px-32">
       {data?.map((card) => (
     ...
     {data?.slice(0,4).map((card) => (
      ...
      <div className="py-10">
          <button onClick={()=> setSeeAll(!seeAll)} className={`block mx-auto ${seeAll && 'hidden'} bg-[#009444] text-white rounded p-1 px-4`}>
            See All
          </button>
        </div>
    </div>
